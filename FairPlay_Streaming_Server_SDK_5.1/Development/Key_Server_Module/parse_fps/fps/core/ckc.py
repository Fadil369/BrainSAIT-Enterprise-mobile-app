...
class CKC(TLLVContainer):
    ...
    def parse_container(self, ckc):
        ...
        with open(ckc, "rb") as f:
            raw_ckc = f.read()
            raw_ckc = base64.b64decode(raw_ckc, validate=False)
    ...
    def generate_encrypted_AR_seed(self, spc):
        if not "sk_r1" in spc.tllvs:
            raise Exception("sk_r1 not found in SPC")
    ...
    def parse(self, raw_ckc, spc=None):
        ...
        try:
            ...
            logging.error("Cannot decrypt CKC payload\n")
            return

        # Parse the CKC payload
        self.parse_TLLVs()
