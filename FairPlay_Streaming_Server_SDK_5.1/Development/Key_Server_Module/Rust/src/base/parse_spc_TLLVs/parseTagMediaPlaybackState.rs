...
impl Base {
    pub fn parseTagMediaPlaybackState(tllv: &FPSServerTLLV, spcContainer: &mut FPSServerSPCContainer) -> Result<()> {
        ...
        // 4B Date
        spcContainer.spcData.playInfo.date = readBigEndianU32(&tllv.value, 0)?;

        // 4B Playback State
        spcContainer.spcData.playInfo.playbackState = readBigEndianU32(&tllv.value, 4)?;

        // 8B Playback ID
        spcContainer.spcData.playInfo.playbackId = readBigEndianU64(&tllv.value, 8)?;
