...
impl Base {
    pub fn populateTagServerOfflineKey(serverCtx: &mut FPSServerCtx) -> Result<()> {
        ...
        if (serverCtx.streamId.is_some()) || (serverCtx.titleId.is_some()) {
        ...
        if let Some(streamId) = serverCtx.streamId.as_mut(){
        ...
        if offlineKeyTLLVVersion == FPS_TLLV_OFFLINEKEY_TLLV_VERSION_2 {
            ...
            offlineKeyTLLV.extend(serverCtx.titleId.as_ref().unwrap());
        }
