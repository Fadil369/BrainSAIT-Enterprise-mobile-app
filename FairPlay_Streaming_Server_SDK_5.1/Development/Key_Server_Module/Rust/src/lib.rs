...
pub extern "C" fn fpsProcessOperations(
    ...
    let result = std::panic::catch_unwind(|| ->  (FPSStatus, usize) {

        let mut output = serde_jsonrc::Value::default();
        ...
        let status = Base::processOperations(root, &mut output);
