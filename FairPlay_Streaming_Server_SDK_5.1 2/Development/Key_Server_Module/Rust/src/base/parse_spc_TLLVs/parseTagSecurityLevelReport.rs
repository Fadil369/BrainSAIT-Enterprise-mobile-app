...
impl Base {
    pub fn parseTagSecurityLevelReport(tllv: &FPSServerTLLV, spcContainer: &mut FPSServerSPCContainer) -> Result<()> {
        ...
        if tllvVersion == 1 {
            ...
            // 8B Security Level
            spcContainer.spcData.supportedSecurityLevel = readBigEndianU64(&tllv.value, 8)?;
            ...
            if spcContainer.spcData.clientKextDenyListVersion == 0 {
                spcContainer.spcData.clientKextDenyListVersion = readBigEndianU32(&tllv.value, 16)?;
            }

            spcContainer.spcData.isSecurityLevelTLLVValid = true;

            log::debug!(
                "Client Security Level: 0x{:x}",
                spcContainer.spcData.supportedSecurityLevel
            );
        }
