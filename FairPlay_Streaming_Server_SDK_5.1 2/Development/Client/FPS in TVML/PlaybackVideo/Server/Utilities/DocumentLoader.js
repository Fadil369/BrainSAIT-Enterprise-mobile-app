...
Abstract:
 The DocumentLoader class handles the loading of templates from the network.
*/

class DocumentLoader {

    constructor(baseURL) {
        console.log("DocumentLoader constructor");

        // Bind the callback methods to the current context.
    ...
    fetch(options) {
        ...
        xhr.onerror = () => {
            if (typeof options.error === "function") {
                ...
                const alertDocument = createLoadErrorAlertDocument(docURL, xhr);
                navigationDocument.presentModal(alertDocument);
            }
        };
        xhr.send();
        // Preserve the request and cancel it on the next fetch.
        if (options.concurrent !== true) {
            this._fetchXHR = xhr;
        }
    }
